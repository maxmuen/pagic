...
const init = new Command().description("Init pagic site/theme/plugin").action(
  async () => {
      ...
      case "theme":
        if (await fs.exists("mod.ts")) {
          const confirmed = await Confirm.prompt(
            "mod.ts already exists, do you want to override it?",
          );
          if (!confirmed) {
            return;
          }
...
await new Command().name("pagic").command("build", build).command("init", init)
  .parse(Deno.args);
